...
def test_create_affiliation(session, auth_mock, monkeypatch):  # pylint:disable=unused-argument
    ...
    # orgId/orgUuid should only appear in nested business.affiliations
    assert "orgId" not in result
    assert "orgUuid" not in result
    nested = result["business"]["affiliations"]
    assert len(nested) > 0
    assert nested[0]["orgId"] == org_id
    assert nested[0]["orgUuid"] == str(org_dictionary["uuid"])
...
def test_create_affiliation_firms(session, auth_mock, monkeypatch):  # pylint:disable=unused-argument
    ...
    affiliation = AffiliationService.create_affiliation(
        org_id, business_identifier, TestEntityInfo.entity_lear_mock3["passCode"]
    )
    assert affiliation
    assert affiliation.entity.identifier == entity_service.identifier
    assert affiliation.as_dict()["organization"]["id"] == org_dictionary["id"]
...
@pytest.mark.parametrize(
    "test_name, org_type, should_succeed",
    [
...
def test_create_affiliation_staff_sbc_staff(
    ...
    # Requires staff role.
    user = (
        TestJwtClaims.staff_role if org_type == OrgType.STAFF.value else TestJwtClaims.public_bceid_account_holder_user
    )
...
def test_find_affiliated_entities_by_org_id(session, auth_mock):  # pylint:disable=unused-argument
    ...
    # create first row in affiliation table
    AffiliationService.create_affiliation(org_id, business_identifier1, TestEntityInfo.entity_lear_mock["passCode"])
    # create second row in affiliation table
    AffiliationService.create_affiliation(org_id, business_identifier2, TestEntityInfo.entity_lear_mock2["passCode"])
...
def test_delete_affiliation(session, auth_mock, monkeypatch):  # pylint:disable=unused-argument
    ...
    with patch.object(ActivityLogPublisher, "publish_activity", return_value=None) as mock_alp:
        ...
        mock_alp.assert_called_with(
            Activity(action=ActivityAction.CREATE_AFFILIATION.value, org_id=ANY, name=ANY, id=ANY)
        )
    ...
    with patch.object(ActivityLogPublisher, "publish_activity", return_value=None) as mock_alp:
        ...
        mock_alp.assert_called_with(
            Activity(action=ActivityAction.REMOVE_AFFILIATION.value, org_id=ANY, name=ANY, id=ANY)
        )
...
def test_delete_affiliation_reset_passcode(session, auth_mock, monkeypatch):  # pylint:disable=unused-argument
    ...
    delete_affiliation_request = DeleteAffiliationRequest(
        org_id=org_id, business_identifier=business_identifier, email_addresses=None, reset_passcode=True
    )
    AffiliationService.delete_affiliation(delete_affiliation_request)
...
def test_create_new_business(session, auth_mock, nr_mock):  # pylint:disable=unused-argument
    ...
    affiliation_data = AffiliationData(
        org_id=org_dictionary["id"], business_identifier="NR 1234567", email="test@test.com", phone="1112223333"
    )
...
def test_find_affiliations_for_new_business(session, auth_mock, nr_mock, monkeypatch):  # pylint:disable=unused-argument
    ...
    for aff in affiliated_entities[0]["affiliations"]:
        assert aff["orgId"] == org_id
        assert aff["orgUuid"] == str(org_dictionary["uuid"])
...
def test_find_affiliation(session, auth_mock):  # pylint:disable=unused-argument
    ...
    # orgId/orgUuid should only appear in nested business.affiliations, not at top level
    assert "orgId" not in affiliation
    assert "orgUuid" not in affiliation
    for nested in affiliation["business"]["affiliations"]:
        assert nested["orgId"] == org_id
        assert nested["orgUuid"] == str(org_dictionary["uuid"])
