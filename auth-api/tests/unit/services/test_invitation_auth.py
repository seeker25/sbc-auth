...
def test_change_authentication_subsequent_invites(session, auth_mock, keycloak_mock, monkeypatch):
    ...
    mock_alp.assert_called_with(
        Activity(
            action=ActivityAction.AUTHENTICATION_METHOD_CHANGE.value,
            org_id=ANY,
            name=ANY,
            id=ANY,
            value=LoginSource.BCSC.value,
        )
    )
    ...
    with patch.object(InvitationService, "send_invitation", return_value=None):
        ...
        with patch.object(ActivityLogPublisher, "publish_activity", return_value=None) as mock_alp:
            ...
            mock_alp.assert_called_with(
                Activity(
                    action=ActivityAction.INVITE_TEAM_MEMBER.value,
                    org_id=ANY,
                    name=invitation_bcsc_info["recipientEmail"],
                    id=ANY,
                    value="USER",
                )
            )
        ...
        with patch.object(ActivityLogPublisher, "publish_activity", return_value=None) as mock_alp:
            ...
            mock_alp.assert_called_with(
                Activity(
                    action=ActivityAction.INVITE_TEAM_MEMBER.value,
                    org_id=ANY,
                    name=invitation_bceid_info["recipientEmail"],
                    id=ANY,
                    value="USER",
                )
            )
...
def test_change_authentication_non_govm(session, auth_mock, keycloak_mock, monkeypatch):
    ...
    with patch.object(InvitationService, "send_invitation", return_value=None):
        ...
        with patch.object(ActivityLogPublisher, "publish_activity", return_value=None) as mock_alp:
            ...
            patch_token_info(
                {
                    "sub": invitee_bcsc_user.keycloak_guid,
                    "idp_userid": invitee_bcsc_user.idp_userid,
    ...
    with patch.object(InvitationService, "send_invitation", return_value=None):
        ...
        with patch.object(ActivityLogPublisher, "publish_activity", return_value=None) as mock_alp:
            ...
            patch_token_info(
                {
                    "sub": invitee_bceid_user.keycloak_guid,
                    "idp_userid": invitee_bceid_user.idp_userid,
...
def test_invitation_govm(session, auth_mock, keycloak_mock, monkeypatch):
    ...
    with patch.object(InvitationService, "send_invitation", return_value=None):
        ...
        with patch.object(ActivityLogPublisher, "publish_activity", return_value=None) as mock_alp:
            ...
            patch_token_info(
                {
                    "sub": staff_invitee_user.keycloak_guid,
                    "idp_userid": staff_invitee_user.idp_userid,
...
def test_invitation_regular_org(session, auth_mock, keycloak_mock, monkeypatch):
    ...
    with patch.object(InvitationService, "send_invitation", return_value=None):
        ...
        with patch.object(ActivityLogPublisher, "publish_activity", return_value=None) as mock_alp:
            ...
            patch_token_info(
                {"sub": invitee_bcsc_user.keycloak_guid, "idp_userid": invitee_bcsc_user.idp_userid}, monkeypatch
            )
            InvitationService.accept_invitation(invitation_dict["id"], User(invitee_bcsc_user), "")
