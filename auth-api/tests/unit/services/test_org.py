...
def test_create_org_products(session, keycloak_mock, monkeypatch):
    ...
    with patch.object(ActivityLogPublisher, "publish_activity", return_value=None) as mock_alp:
        ...
        mock_alp.assert_called_once_with(
            Activity(
...
def test_put_basic_org_assert_pay_request_is_govm(session, keycloak_mock, staff_user_mock, monkeypatch):  # pylint:disable=unused-argument
    ...
    staff_token_info = TestJwtClaims.get_test_user(
        sub=user.keycloak_guid, source=LoginSource.STAFF.value, roles=["create_accounts"], idp_userid=user.idp_userid
    )
    user2 = factory_user_model(TestUserInfo.user2)
    public_token_info = TestJwtClaims.get_test_user(
        sub=user2.keycloak_guid, source=LoginSource.STAFF.value, roles=["gov_account_user"], idp_userid=user2.idp_userid
    )
    ...
    with patch.object(RestService, "put") as mock_post:
        ...
        org_body = {"mailingAddress": TestOrgInfo.get_mailing_address(), **payment_details}
        patch_token_info(public_token_info, monkeypatch)
        with patch.object(ActivityLogPublisher, "publish_activity", return_value=None) as mock_alp:
            org = OrgService.update_org(org, org_body)
            mock_alp.assert_called_with(
                Activity(action=ActivityAction.ACCOUNT_ADDRESS_CHANGE.value, org_id=ANY, name=ANY, id=ANY, value=ANY)
            )
            assert org
        dictionary = org.as_dict()
        assert dictionary["name"] == TestOrgInfo.org_govm["name"]
...
def test_create_product_single_subscription(session, keycloak_mock, monkeypatch):  # pylint:disable=unused-argument
    ...
    subscriptions = ProductService.create_product_subscription(
        dictionary["id"], TestOrgProductsInfo.org_products1, skip_auth=True
    )
    assert next(
        prod
        for prod in subscriptions
...
def test_create_product_single_subscription_duplicate_error(session, keycloak_mock, monkeypatch):  # pylint:disable=unused-argument
    ...
    patch_token_info(TestJwtClaims.public_bceid_user, monkeypatch)
    subscriptions = ProductService.create_product_subscription(
        dictionary["id"], TestOrgProductsInfo.org_products_business, skip_auth=True
    )
    assert next(
        prod
        for prod in subscriptions
...
@pytest.mark.parametrize("org_type", [(OrgType.STAFF.value), (OrgType.SBC_STAFF.value)])
@mock.patch("auth_api.services.affiliation_invitation.RestService.get_service_account_token", mock_token)
def test_create_product_subscription_staff(session, keycloak_mock, org_type, monkeypatch):
    ...
    subscriptions = ProductService.create_product_subscription(
        org._model.id, TestOrgProductsInfo.org_products2, skip_auth=True
    )
...
def test_create_product_subscription_nds(session, keycloak_mock, monkeypatch):  # pylint:disable=unused-argument
    ...
    subscriptions = ProductService.create_product_subscription(
        org_dict["id"], TestOrgProductsInfo.org_products_nds, skip_auth=False
    )
    assert next(
        prod
        for prod in subscriptions
...
def test_update_org_name(session, monkeypatch):  # pylint:disable=unused-argument
    ...
    with patch.object(RestService, "put") as mock_put:
        with patch.object(ActivityLogPublisher, "publish_activity", return_value=None) as mock_alp:
            ...
            mock_alp.assert_called_with(
                Activity(action=ActivityAction.ACCOUNT_NAME_CHANGE.value, org_id=ANY, value="My Test", id=ANY, name=ANY)
            )
        assert org
        dictionary = org.as_dict()
        mock_put.assert_called()
        actual_data = mock_put.call_args.kwargs.get("data")
        expected_data = {"accountId": dictionary.get("id"), "accountName": dictionary.get("name"), "branchName": ""}
...
def test_suspend_org(session, monkeypatch):  # pylint:disable=unused-argument
    ...
    patch_token_info(token_info, monkeypatch)
    updated_org = org.change_org_status(OrgStatus.SUSPENDED.value, SuspensionReasonCode.OWNER_CHANGE.name)
    ...
    updated_org = org.change_org_status(OrgStatus.ACTIVE.value, SuspensionReasonCode.DISPUTE.name)
...
def test_get_members(session, keycloak_mock, monkeypatch):  # pylint:disable=unused-argument
    ...
    patch_token_info(TestJwtClaims.public_user_role, monkeypatch)
    response = MembershipService.get_members_for_org(org_dictionary["id"], status="ACTIVE")
    assert response
...
def test_delete_org_with_affiliation(publish_mock, session, auth_mock, keycloak_mock, monkeypatch):  # pylint:disable=unused-argument
    ...
    publish_mock.assert_called_once_with(QueueMessageTypes.BUSINESS_UNAFFILIATED.value, org_id, business_identifier)
...
def test_patch_org_status(session, monkeypatch, auth_mock):  # pylint:disable=unused-argument
    ...
    with patch.object(ActivityLogPublisher, "publish_activity", return_value=None) as mock_alp:
        ...
        mock_alp.assert_called_with(
            Activity(
                action=ActivityAction.ACCOUNT_SUSPENSION.value,
                org_id=ANY,
                name=ANY,
                id=ANY,
    ...
    with patch.object(ActivityLogPublisher, "publish_activity", return_value=None) as mock_alp:
        ...
        mock_alp.assert_called_with(
            Activity(
                action=ActivityAction.AUTHENTICATION_METHOD_CHANGE.value, org_id=ANY, name=ANY, id=ANY, value="BCROS"
            )
        )
...
def test_patch_org_access_type(session, monkeypatch):  # pylint:disable=unused-argument
    ...
    # Validate and update org status
    patch_info = {"action": PatchActions.UPDATE_ACCESS_TYPE.value}
    with pytest.raises(BusinessException) as exception:
...
def test_create_product_single_subscription_qs(session, monkeypatch):
    ...
    ProductService.create_product_subscription(
        org_id=dictionary["id"], subscription_data=TestOrgProductsInfo.mhr_qs_lawyer_and_notaries, skip_auth=True
    )

    org_subscriptions = ProductSubscriptionModel.find_by_org_ids(org_ids=[dictionary["id"]])
    org_prod_sub = next(prod for prod in org_subscriptions if prod.product_code == product_code)
