        ...
        with patch.object(auth, "check_auth", return_value=True):
            with patch.object(InvitationService, "notify_admin", return_value=None):
                ...
                with patch.object(ActivityLogPublisher, "publish_activity", return_value=None) as mock_alp:
                    ...
                    mock_alp.assert_called_with(
                        Activity(
                            action=ActivityAction.APPROVE_TEAM_MEMBER.value, org_id=ANY, name=ANY, id=ANY, value=ANY
                        )
                    )
    ...
    with patch.object(InvitationService, "send_invitation", return_value=None):
        with patch.object(auth, "check_auth", return_value=True):
            with patch.object(InvitationService, "notify_admin", return_value=None):
                ...
                with pytest.raises(BusinessException) as exception:
                    expired_invitation: InvitationModel = InvitationModel.find_invitation_by_id(
                        new_invitation_dict["id"]
                    )
...
def test_get_invitations_by_org_id(session, auth_mock, keycloak_mock, monkeypatch):  # pylint:disable=unused-argument
    ...
    with patch.object(InvitationService, "send_invitation", return_value=None):
        ...
        invitations: list = InvitationService.get_invitations_for_org(
            org_id, status="PENDING", token_info=TestJwtClaims.public_user_role
        )
        assert invitations
